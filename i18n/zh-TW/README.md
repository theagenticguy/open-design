...
> **[Claude Design][cd] 的開源替代品。** 本地優先、每一層都 BYOK —— **10 套 coding-agent CLI** 在 `PATH` 上自動檢測（Claude Code、Codex、Cursor Agent、Gemini CLI、OpenCode、Qwen、GitHub Copilot CLI、Hermes、Kimi、Pi）就是設計引擎，由 **31 個可組合 Skills** 和 **70 套品牌級 Design System** 驅動。一個都沒裝？還有 OpenAI 相容的 BYOK 代理 `/api/proxy/stream` 備援，同一條 loop，少一次 spawn 而已。
...
<p align="center"><a href="../../README.md">English</a> · <a href="../de/README.md">Deutsch</a> · <a href="../zh-CN/README.md">简体中文</a> · <b>繁體中文</b> · <a href="../ko/README.md">한국어</a> · <a href="../ja/README.md">日本語</a></p>

---

## 為什麼要做這個

Anthropic 的 [Claude Design][cd]（2026-04-17 釋出，基於 Opus 4.7）讓大家第一次看到：當一個 LLM 不再寫廢話、開始直接交付設計成品，會是什麼樣子。它瞬間爆紅 —— 然後保持**閉源**、付費、只跑在雲上、綁定 Anthropic 的模型和 Anthropic 的內部 skill。沒有 checkout，沒有自託管，也換不了自己的 agent。

**Open Design（OD）就是它的開源替代品。** 同一套 loop、同一種「artifact-first」心智模型，但沒有鎖定。我們不做 agent —— 你筆記本上最強的 coding agent 已經裝好了。我們要做的，是把它接進一個 skill 驅動的設計工作流：本地用 `pnpm tools-dev` 跑完整本地閉環，每一層都 BYOK（自帶 Key）。
...
| **內建 design system** | **70 套** —— 2 套手寫起手 + 68 套從 [`awesome-design-md`][acd2] 匯入的產品系統（Linear、Stripe、Airbnb、Tesla、Notion、Anthropic、Apple、Cursor、Figma、小紅書…） |
...
| **部署目標** | 本地 `pnpm tools-dev` · 打包 Electron（`apps/packaged/` 佔位中） |
| **License** | Apache-2.0 |
...
[`VoltAgent/awesome-design-md`][acd2] 的 9 段式 `DESIGN.md` —— color、typography、spacing、layout、components、motion、voice、brand、anti-patterns。每個 artifact 都從啟用的 system 裡讀 token。切換 system → 下一次渲染就用新的 token。下拉框裡現成的有：**Linear、Stripe、Airbnb、Tesla、Notion、Apple、Anthropic、Cursor、Figma、Resend、Raycast、Lovable、Cohere、Mistral、ElevenLabs、X.AI、Spotify、Webflow、Sanity、PostHog、Sentry、MongoDB、ClickHouse、Cal、Replicate、Clay、Composio、小紅書…** 共 70 套。
...
**開發者工具** —— `cursor` · `linear-app` · `framer` · `expo` · `clickhouse` · `mongodb` · `hashicorp` · `posthog` · `sentry` · `warp` · `webflow` · `sanity` · `mintlify` · `lovable` · `composio` · `opencode-ai` · `voltagent`
...
| Modern minimal — Linear | 冷調、結構化、剋制強調 | Linear · Stripe |
...
| Design system | 私有 | `DESIGN.md`（v0.2 路線圖） | **`DESIGN.md` × 70 套，開箱即有** |
...
- [x] 31 個 skill + 70 套 design system + 5 套視覺方向 + 5 個裝置外殼
