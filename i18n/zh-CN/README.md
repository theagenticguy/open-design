...
> **[Claude Design][cd] 的开源替代品。** 本地优先、每一层都 BYOK —— **10 套 coding-agent CLI** 在 `PATH` 上自动检测（Claude Code、Codex、Cursor Agent、Gemini CLI、OpenCode、Qwen、GitHub Copilot CLI、Hermes、Kimi、Pi）就是设计引擎，由 **31 个可组合 Skills** 和 **70 套品牌级 Design System** 驱动。一个都没装？还有 OpenAI 兼容的 BYOK 代理 `/api/proxy/stream` 兜底，同一条 loop，少一次 spawn 而已。
...
<p align="center"><a href="../../README.md">English</a> · <a href="../de/README.md">Deutsch</a> · <b>简体中文</b> · <a href="../zh-TW/README.md">繁體中文</a> · <a href="../ko/README.md">한국어</a> · <a href="../ja/README.md">日本語</a></p>

---

## 为什么要做这个

Anthropic 的 [Claude Design][cd]（2026-04-17 发布，基于 Opus 4.7）让大家第一次看到：当一个 LLM 不再写废话、开始直接交付设计成品，会是什么样子。它瞬间出圈 —— 然后保持**闭源**、付费、只跑在云上、绑定 Anthropic 的模型和 Anthropic 的内部 skill。没有 checkout，没有自托管，也换不了自己的 agent。

**Open Design（OD）就是它的开源替代品。** 同一套 loop、同一种「artifact-first」心智模型，但没有锁定。我们不做 agent —— 你笔记本上最强的 coding agent 已经装好了。我们要做的，是把它接进一个 skill 驱动的设计工作流：本地用 `pnpm tools-dev` 跑完整本地闭环，每一层都 BYOK（自带 Key）。
...
| **内置 design system** | **70 套** —— 2 套手写起手 + 68 套从 [`awesome-design-md`][acd2] 导入的产品系统（Linear、Stripe、Airbnb、Tesla、Notion、Anthropic、Apple、Cursor、Figma、小红书…） |
...
| **部署目标** | 本地 `pnpm tools-dev` · 打包 Electron（`apps/packaged/` 占位中） |
| **License** | Apache-2.0 |
...
[`VoltAgent/awesome-design-md`][acd2] 的 9 段式 `DESIGN.md` —— color、typography、spacing、layout、components、motion、voice、brand、anti-patterns。每个 artifact 都从激活的 system 里读 token。切换 system → 下一次渲染就用新的 token。下拉框里现成的有：**Linear、Stripe、Airbnb、Tesla、Notion、Apple、Anthropic、Cursor、Figma、Resend、Raycast、Lovable、Cohere、Mistral、ElevenLabs、X.AI、Spotify、Webflow、Sanity、PostHog、Sentry、MongoDB、ClickHouse、Cal、Replicate、Clay、Composio、小红书…** 共 70 套。
...
**开发者工具** —— `cursor` · `linear-app` · `framer` · `expo` · `clickhouse` · `mongodb` · `hashicorp` · `posthog` · `sentry` · `warp` · `webflow` · `sanity` · `mintlify` · `lovable` · `composio` · `opencode-ai` · `voltagent`
...
| Modern minimal — Linear | 冷调、结构化、克制强调 | Linear · Stripe |
...
| Design system | 私有 | `DESIGN.md`（v0.2 路线图） | **`DESIGN.md` × 70 套，开箱即有** |
...
- [x] 31 个 skill + 70 套 design system + 5 套视觉方向 + 5 个设备外壳
